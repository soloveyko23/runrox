	...
	(s.g = (function () {
		...
		(() => {
			...
			let Ye = (e, t = !1, s = 500, i = 0) => {
					...
					} else d(`[gotoBlock]: Йой... Такого блоку немає на сторінці: ${e}`);
				},
				Xe = !1;
					...
					(this.оbjects = []),
						...
						this.mediaQueries.forEach((e) => {
							const t = e.split(","),
								...
								i = t[1],
								r = this.оbjects.filter(({ breakpoint: e }) => e === i);
			...
			(e.popup = new (class {
				constructor(e) {
					...
					this.youTubeCode,
						...
						(this.bodyLock = !1),
						console.log(
							`[POPUP DEBUG]: Конструктор попапа выполнен, options.init: ${this.options.init}`,
						),
				...
				eventsPopup() {
					this.popupLogging("Налаштування подій Popup"),
						document.addEventListener(
							...
							function (e) {
								console.log(
									`[POPUP DEBUG]: Клик обнаружен, target: ${e.target.tagName}.${e.target.className}`,
								);
								...
								if (t)
									return (
										this.popupLogging(
											`Клік по кнопці відкриття: ${t.getAttribute(
												this.options.attributeOpenButton,
											)}`,
										),
										e.preventDefault(),
										...
										"error" !== this._dataValue
											...
											: void this.popupLogging(
													`Помилка: не заповнено атрибут у ${t.classList}`,
											  )
									);
								...
								return s ||
									...
									? (this.popupLogging(
											"Спроба закриття: " +
												(s ? "кнопка data-close" : "клік поза popup__content"),
									  ),
									  e.preventDefault(),
									  void this.close())
									: void 0;
						...
						this.options.hashSettings.goHash &&
							(window.addEventListener(
								...
								function () {
									window.location.hash
										? (this.popupLogging(
												`Хеш змінився: ${window.location.hash}`,
										  ),
										  this._openToHash())
										: (this.popupLogging(
												`Хеш видалено, закриваю попап: ${this.targetOpen.selector}`,
										  ),
							...
							window.addEventListener(
								...
								function () {
									window.location.hash &&
										(this.popupLogging(
											`Завантаження сторінки з хешем: ${window.location.hash}`,
										),
										this._openToHash());
								}.bind(this),
							));
				...
				open(e) {
					if (
						(this.popupLogging(
							`Метод open викликано з selectorValue: ${
						...
						if (
							...
							if (this.youTubeCode) {
								...
								t.setAttribute("allow", `${s}; encrypted-media`),
									...
									this.targetOpen.element
										...
										?.appendChild(t),
									this.popupLogging(`Додано YouTube iframe: ${e}`);
							}
							const e = this.targetOpen.element.querySelectorAll("video");
							e.length > 0 &&
								(this.popupLogging(
									`Знайдено ${e.length} відео для автозапуску`,
								),
								e.forEach((e) => {
									e.play().catch(() => {
										this.popupLogging(
											"Відео не запустилось (можливо через політику браузера)",
										);
									});
								})),
								...
								(this.isOpen = !0),
								this.popupLogging(
									`Попап відкрито, isOpen: ${this.isOpen}, selector: ${this.targetOpen.selector}`,
								),
								setTimeout(() => {
									this._focusTrap();
								}, 50),
							...
							this.popupLogging(
								`Помилка: попап ${this.targetOpen.selector} не знайдено`,
							);
					else this.popupLogging(`Помилка: bodyLockStatus = ${a}`);
				}
				close(e) {
					if (
						(this.popupLogging(
							`Метод close викликано з selectorValue: ${e || "немає"}`,
						),
						e &&
							"string" == typeof e &&
							"" !== e.trim() &&
							...
							this.popupLogging(
								`Оновлено previousOpen.selector: ${this.previousOpen.selector}`,
							)),
						this.isOpen && a)
					) {
						if (
						...
						this.previousOpen.element &&
							...
							this.popupLogging(
								`Видалено клас ${this.options.classes.popupActive} з ${
							...
							this._reopen ||
								...
								(this.isOpen = !1),
								this.popupLogging(
									`isOpen встановлено в ${this.isOpen}, bodyUnlock викликано`,
								)),
							this._removeHash(),
							this._selectorOpen &&
								...
								this.popupLogging(
									`Оновлено lastClosed: ${this.lastClosed.selector}`,
								)),
						...
						this.popupLogging(
							`Вихід з close: isOpen=${this.isOpen}, bodyLockStatus=${a}`,
						);
				}
				_stopAllVideos() {
					if (!this.targetOpen.element) return;
					this.popupLogging(
						`Остановка видео в попапе: ${this.targetOpen.selector}`,
					);
					const e = this.targetOpen.element.querySelectorAll("video");
					e.forEach((e) => {
					...
					t.forEach((e) => {
						...
						this.popupLogging(
							`Остановлено ${e.length} видео и ${t.length} YouTube iframe`,
						);
				}
				_getHash() {
					this.options.hashSettings.location &&
							...
							if (s.closest("[data-goto]")) {
								...
								if (
									...
									document.querySelector(l)
										? Ye(l, n, a, o)
										: console.warn(
												`Элемент с селектором "${l}" не найден на странице`,
										  );
								}
								if (l) {
									let e = "";
