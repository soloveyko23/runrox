(() => {
    ...
    (() => {
        ...
        function addLoadedClass() {
            if (!document.documentElement.classList.contains("loading")) {
                ...
                window.addEventListener("load", (function() {
                    if (!document.documentElement.classList.contains("loaded")) document.documentElement.classList.add("loaded");
                }));
            }
        }
        function getHash() {
        ...
        class BeforeAfter {
            ...
            init() {
                ...
                if (beforeAfterItems.length > 0) {
                    this.setLogging(`Found ${beforeAfterItems.length} elements`);
                    this.initItems(beforeAfterItems);
                }
            }
            initItems(items) {
                items.forEach(((element, index) => {
                    ...
                    const instance = {
                        ...
                        isDragging: false,
                        wasDragging: false,
                        arrow: element.querySelector("[data-ba-arrow]"),
                        after: element.querySelector("[data-ba-after]"),
                        isInitialized: false
                    };
                    if (!instance.arrow || !instance.after) return;
                    this.instances.push(instance);
                    this.setupElement(instance);
                    this.bindEvents(instance);
                    const savedPosition = this.getSavedPosition(instance);
                    this.updatePosition(instance, savedPosition);
                    instance.isInitialized = true;
                }));
            }
            getSavedPosition(instance) {
                const afterElement = instance.after;
                const computedStyle = window.getComputedStyle(afterElement);
                const currentWidth = parseFloat(computedStyle.width);
                const parentWidth = afterElement.parentElement.offsetWidth;
                if (parentWidth > 0 && currentWidth > 0) {
                    const currentPosition = 100 - currentWidth / parentWidth * 100;
                    if (Math.abs(currentPosition - 50) > 1) return currentPosition;
                }
                return 50;
            }
            setupElement(instance) {
                const {arrow, element} = instance;
                arrow.style.touchAction = "none";
                arrow.style.userSelect = "none";
                arrow.style.webkitUserSelect = "none";
                arrow.style.msUserSelect = "none";
                arrow.style.webkitTouchCallout = "none";
                arrow.style.webkitTapHighlightColor = "transparent";
                arrow.style.cursor = "ew-resize";
            ...
            bindEvents(instance) {
                ...
                const interactiveElement = isMobileDevice ? element : arrow;
                interactiveElement.addEventListener("touchstart", (e => this.onStart(e, instance)), {
                    passive: false
                });
                interactiveElement.addEventListener("touchmove", (e => this.onMove(e, instance)), {
                    passive: false
                });
                interactiveElement.addEventListener("touchend", (e => this.onEnd(e, instance)), {
                    passive: false
                });
                interactiveElement.addEventListener("touchcancel", (e => this.onEnd(e, instance)), {
                    passive: false
                });
                if (!isMobileDevice) arrow.addEventListener("mousedown", (e => this.onStart(e, instance)));
                if (isMobileDevice) element.addEventListener("click", (e => this.onSimpleClick(e, instance)));
                arrow.addEventListener("dragstart", (e => e.preventDefault()));
                arrow.addEventListener("contextmenu", (e => e.preventDefault()));
                element.addEventListener("keydown", (e => this.onKeyboard(e, instance)));
                element.addEventListener("dblclick", (() => this.reset(instance)));
                element.addEventListener("focus", (() => this.onFocus(instance)));
                element.addEventListener("blur", (() => this.onBlur(instance)));
            }
            onFocus(instance) {
                this.stopScrollHint();
                this.saveCurrentPosition(instance);
            }
            onBlur(instance) {
                this.stopScrollHint();
                this.saveCurrentPosition(instance);
            }
            saveCurrentPosition(instance) {
                instance.element.setAttribute("data-ba-position", instance.position.toString());
            }
            onStart(e, instance) {
                e.preventDefault();
                e.stopPropagation();
                ...
                instance.element.classList.add("_dragging");
                this.saveCurrentPosition(instance);
                if (e.type === "mousedown") {
                    this.mouseMoveHandler = moveEvent => this.onMove(moveEvent, instance);
                    this.mouseUpHandler = upEvent => this.onEnd(upEvent, instance);
            ...
            onMove(e, instance) {
                ...
                e.preventDefault();
                e.stopPropagation();
                instance.wasDragging = true;
                const rect = instance.element.getBoundingClientRect();
                const clientX = this.getClientX(e);
                const relativeX = clientX - rect.left;
                const percentage = Math.max(0, Math.min(100, relativeX / rect.width * 100));
                this.updatePosition(instance, percentage);
            }
            onEnd(e, instance) {
                if (!instance.isDragging) return;
                e.preventDefault();
                ...
                instance.element.classList.remove("_dragging");
                this.saveCurrentPosition(instance);
            ...
            onKeyboard(e, instance) {
                const step = 5;
                let newPosition = instance.position;
                switch (e.key) {
                  case "ArrowLeft":
                    e.preventDefault();
                    newPosition = Math.max(0, instance.position - step);
                    break;

                  case "ArrowRight":
                    e.preventDefault();
                    newPosition = Math.min(100, instance.position + step);
                    break;
                ...
                this.updatePosition(instance, newPosition);
                this.saveCurrentPosition(instance);
            }
            reset(instance) {
                this.updatePosition(instance, 50);
                this.saveCurrentPosition(instance);
            }
            updatePosition(instance, percentage) {
                instance.position = percentage;
                const {arrow, after} = instance;
                const arrowWidth = arrow.offsetWidth;
                arrow.style.left = `calc(${percentage}% - ${arrowWidth / 2}px)`;
                after.style.width = `${100 - percentage}%`;
                if (instance.isInitialized) this.saveCurrentPosition(instance);
            }
            getClientX(e) {
            ...
            setLogging(message) {
                if (this.config.logging) console.log(`[BeforeAfter]: ${message}`);
            }
            destroy() {
            ...
            stopScrollHint() {
                if (window.scrollHintManager) window.scrollHintManager.hideAll();
            }
            onSimpleClick(e, instance) {
                if (instance.wasDragging) {
                    instance.wasDragging = false;
                    return;
                }
                e.preventDefault();
                ...
                const relativeX = clientX - rect.left;
                const percentage = Math.max(0, Math.min(100, relativeX / rect.width * 100));
                this.animateToPosition(instance, percentage);
            }
            animateToPosition(instance, targetPosition) {
                ...
                const animate = currentTime => {
                    ...
                    this.updatePosition(instance, currentPosition);
                    if (progress < 1) requestAnimationFrame(animate); else this.saveCurrentPosition(instance);
                };
        ...
        function initSliders() {
            ...
            const tradingSliderEl = document.querySelector(".trading-indicators__swiper");
            if (tradingSliderEl) {
                let userHasInteracted = false;
                new swiper_core_Swiper(".trading-indicators__swiper", {
                    ...
                    on: {
                        init: function() {
                            if (window.innerWidth <= 479.98) tradingSliderEl.classList.add("_scrolling");
                        },
                        touchStart: function() {
                            if (window.innerWidth <= 479.98 && !userHasInteracted) {
                                userHasInteracted = true;
                                tradingSliderEl.classList.remove("_scrolling");
                            }
                        },
                ...
                function handleResize() {
                    if (window.innerWidth <= 479.98) {
                        if (!userHasInteracted) tradingSliderEl.classList.add("_scrolling");
                    } else {
                        tradingSliderEl.classList.remove("_scrolling");
            ...
            function initTariffCardSlider() {
                const tariffCardsContainers = document.querySelectorAll(".tariff-cards");
                if (tariffCardsContainers.length) tariffCardsContainers.forEach((container => {
                    new swiper_core_Swiper(container, {
                        modules: [ Pagination ],
                        observer: true,
                        observeParents: true,
                        slidesPerView: 1.2,
        ...
        window.addEventListener("load", (() => {
            flsModules.watcher.onDOMContentLoaded();
        }));
            ...
            function pageNavigationAction(e) {
                if (e.type === "click") {
                    ...
                    if (targetElement.closest("[data-goto]")) {
                        ...
                        if (gotoLinkSelector.includes(".html")) {
                            window.location.href = gotoLinkSelector;
                            e.preventDefault();
                            return;
                        }
                        let scrollSelector = gotoLinkSelector;
                        if (gotoLinkSelector.includes("#")) {
                            const hashPart = gotoLinkSelector.split("#")[1];
                            scrollSelector = "#" + hashPart;
                        }
                        const allNavigatorItems = document.querySelectorAll("[data-goto]._navigator-active");
                        allNavigatorItems.forEach((item => item.classList.remove("_navigator-active")));
                        gotoLink.classList.add("_navigator-active");
                        if (flsModules.fullpage) {
                            const fullpageSection = document.querySelector(`${scrollSelector}`).closest("[data-fp-section]");
                            ...
                            const targetElement = document.querySelector(scrollSelector);
                            if (targetElement) gotoBlock(scrollSelector, noHeader, gotoSpeed, offsetTop); else console.warn(`Элемент с селектором "${scrollSelector}" не найден на странице`);
                        }
                        if (scrollSelector) {
                            let hashValue = "";
                            if (scrollSelector.includes("#")) hashValue = scrollSelector.split("#")[1]; else if (scrollSelector.startsWith(".") || scrollSelector.startsWith("#")) hashValue = scrollSelector.substring(1); else hashValue = scrollSelector;
                            if (hashValue) if (history.pushState) {
                    ...
                    if (targetElement.dataset.watch === "navigator") {
                        ...
                        if (entry.isIntersecting) {
                            ...
                            if (navigatorCurrentItem) {
                                const gotoSelector = navigatorCurrentItem.dataset.goto;
                                if (gotoSelector) {
                                    let hashValue = "";
                                    if (gotoSelector.includes("#")) hashValue = gotoSelector.split("#")[1]; else if (gotoSelector.startsWith(".") || gotoSelector.startsWith("#")) hashValue = gotoSelector.substring(1); else hashValue = gotoSelector;
                                    if (hashValue) if (history.replaceState) {
                                        const newUrl = window.location.pathname + window.location.search + "#" + hashValue;
        ...
        class Popup {
            constructor(options) {
                ...
                this.bodyLock = false;
                console.log(`[POPUP DEBUG]: Конструктор попапа выполнен, options.init: ${this.options.init}`);
                this.options.init ? this.initPopups() : null;
            }
            initPopups() {
                console.log(`[POPUP DEBUG]: Ініціалізація Popup СТАРТОВАНА`);
                this.popupLogging(`Ініціалізація Popup`);
                console.log(`[POPUP DEBUG]: bodyLockStatus: ${bodyLockStatus}`);
                this.popupLogging(`bodyLockStatus: ${bodyLockStatus}`);
                this.eventsPopup();
                console.log(`[POPUP DEBUG]: Ініціалізація Popup ЗАВЕРШЕНА`);
            }
            eventsPopup() {
                this.popupLogging(`Налаштування подій Popup`);
                document.addEventListener("click", function(e) {
                    console.log(`[POPUP DEBUG]: Клик обнаружен, target: ${e.target.tagName}.${e.target.className}`);
                    const buttonOpen = e.target.closest(`[${this.options.attributeOpenButton}]`);
                    if (buttonOpen) {
                        this.popupLogging(`Клік по кнопці відкриття: ${buttonOpen.getAttribute(this.options.attributeOpenButton)}`);
                        e.preventDefault();
                        ...
                        if (this._dataValue !== "error") {
                            ...
                            this.open();
                            return;
                        } else this.popupLogging(`Помилка: не заповнено атрибут у ${buttonOpen.classList}`);
                        return;
                    }
                    const buttonClose = e.target.closest(`[${this.options.attributeCloseButton}]`);
                    if (buttonClose || !e.target.closest(`.${this.options.classes.popupContent}`) && this.isOpen) {
                        this.popupLogging(`Спроба закриття: ${buttonClose ? "кнопка data-close" : "клік поза popup__content"}`);
                        e.preventDefault();
                        this.close();
                        return;
                    }
                ...
                document.addEventListener("keydown", function(e) {
                    if (this.options.closeEsc && e.which == 27 && e.code === "Escape" && this.isOpen) {
                        this.popupLogging(`Спроба закриття через ESC`);
                        e.preventDefault();
                        this.close();
                        return;
                    }
                ...
                if (this.options.hashSettings.goHash) {
                    window.addEventListener("hashchange", function() {
                        if (window.location.hash) {
                            this.popupLogging(`Хеш змінився: ${window.location.hash}`);
                            this._openToHash();
                        } else {
                            this.popupLogging(`Хеш видалено, закриваю попап: ${this.targetOpen.selector}`);
                            this.close(this.targetOpen.selector);
                        }
                    }.bind(this));
                    window.addEventListener("load", function() {
                        if (window.location.hash) {
                            this.popupLogging(`Завантаження сторінки з хешем: ${window.location.hash}`);
                            this._openToHash();
                        }
                    }.bind(this));
                }
            ...
            open(selectorValue) {
                this.popupLogging(`Метод open викликано з selectorValue: ${selectorValue || this.targetOpen.selector}`);
                if (bodyLockStatus) {
                    ...
                    this.popupLogging(`targetOpen.element: ${this.targetOpen.element ? this.targetOpen.element.id : "не знайдено"}`);
                    if (this.targetOpen.element) {
                        if (this.youTubeCode) {
                            ...
                            this.popupLogging(`Додано YouTube iframe: ${urlVideo}`);
                        }
                        const videos = this.targetOpen.element.querySelectorAll("video");
                        if (videos.length > 0) {
                            this.popupLogging(`Знайдено ${videos.length} відео для автозапуску`);
                            videos.forEach((video => {
                                video.play().catch((() => {
                                    this.popupLogging(`Відео не запустилось (можливо через політику браузера)`);
                                }));
                            }));
                        }
                        ...
                        this._selectorOpen = false;
                        this.isOpen = true;
                        this.popupLogging(`Попап відкрито, isOpen: ${this.isOpen}, selector: ${this.targetOpen.selector}`);
                        setTimeout((() => {
                            this._focusTrap();
                        }), 50);
                    ...
                    } else this.popupLogging(`Помилка: попап ${this.targetOpen.selector} не знайдено`);
                } else this.popupLogging(`Помилка: bodyLockStatus = ${bodyLockStatus}`);
            }
            close(selectorValue) {
                this.popupLogging(`Метод close викликано з selectorValue: ${selectorValue || "немає"}`);
                if (selectorValue && typeof selectorValue === "string" && selectorValue.trim() !== "") {
                    this.previousOpen.selector = selectorValue;
                    this.popupLogging(`Оновлено previousOpen.selector: ${this.previousOpen.selector}`);
                }
                if (!this.isOpen || !bodyLockStatus) {
                    this.popupLogging(`Вихід з close: isOpen=${this.isOpen}, bodyLockStatus=${bodyLockStatus}`);
                    return;
                }
                ...
                this._stopAllVideos();
                if (this.youTubeCode) {
                    const youtubeContainer = this.targetOpen.element?.querySelector(`[${this.options.youtubePlaceAttribute}]`);
                    if (youtubeContainer) {
                        youtubeContainer.innerHTML = "";
                        this.popupLogging(`Очищено YouTube контейнер`);
                    }
                }
                if (this.previousOpen.element) {
                    this.previousOpen.element.classList.remove(this.options.classes.popupActive);
                    this.previousOpen.element.setAttribute("aria-hidden", "true");
                    this.popupLogging(`Видалено клас ${this.options.classes.popupActive} з ${this.previousOpen.element.id || this.previousOpen.element.className}`);
                }
                if (!this._reopen) {
                    document.documentElement.classList.remove(this.options.classes.bodyActive);
                    !this.bodyLock ? bodyUnlock() : null;
                    this.isOpen = false;
                    this.popupLogging(`isOpen встановлено в ${this.isOpen}, bodyUnlock викликано`);
                }
                this._removeHash();
                if (this._selectorOpen) {
                    this.lastClosed.selector = this.previousOpen.selector;
                    this.lastClosed.element = this.previousOpen.element;
                    this.popupLogging(`Оновлено lastClosed: ${this.lastClosed.selector}`);
                }
            ...
            _stopAllVideos() {
                ...
                this.popupLogging(`Остановка видео в попапе: ${this.targetOpen.selector}`);
                const videos = this.targetOpen.element.querySelectorAll("video");
                videos.forEach((video => {
                    if (!video.paused) {
                        video.pause();
                ...
                const youtubeIframes = this.targetOpen.element.querySelectorAll('iframe[src*="youtube"]');
                youtubeIframes.forEach((iframe => {
                    try {
                        iframe.contentWindow?.postMessage('{"event":"command","func":"pauseVideo","args":""}', "*");
                    } catch (e) {}
                }));
                this.popupLogging(`Остановлено ${videos.length} видео и ${youtubeIframes.length} YouTube iframe`);
            }
            _getHash() {
                if (this.options.hashSettings.location) {
                    this.hash = this.targetOpen.selector.includes("#") ? this.targetOpen.selector : this.targetOpen.selector.replace(".", "#");
                    this.popupLogging(`Отримано хеш: ${this.hash}`);
                }
            }
            _openToHash() {
                ...
                this.popupLogging(`Відкриття по хешу: ${classInHash}, кнопка: ${buttons ? buttons.getAttribute(this.options.attributeOpenButton) : "не знайдено"}`);
                if (buttons && classInHash) this.open(classInHash);
            }
            _setHash() {
                this.popupLogging(`Встановлено хеш: ${this.hash}`);
                history.pushState("", "", this.hash);
            }
            _removeHash() {
                this.popupLogging(`Видалено хеш`);
        ...
        window.scrollHintManager = {
            ...
            show: function(slider) {
                if (this.isActive || !slider || this.hasBeenShown) return;
                slider.classList.add("_scrolling");
                this.isActive = true;
                this.currentSlider = slider;
                this.hasBeenShown = true;
            },
            hide: function() {
                if (!this.isActive || !this.currentSlider) return;
                this.currentSlider.classList.remove("_scrolling");
                this.isActive = false;
                this.currentSlider = null;
            },
            hideAll: function() {
                const allSliders = document.querySelectorAll(".trading-indicators__swiper._scrolling");
                allSliders.forEach((slider => slider.classList.remove("_scrolling")));
                this.isActive = false;
                this.currentSlider = null;
            }
        };
        function initTradingScrollHint() {
            const tradingSliders = document.querySelectorAll(".trading-indicators__swiper");
            if (!tradingSliders.length || window.innerWidth >= 1024) return;
            ...
            observer = new IntersectionObserver((entries => {
                const sortedEntries = entries.sort(((a, b) => {
                    const aIndex = Array.from(tradingSliders).indexOf(a.target);
                    const bIndex = Array.from(tradingSliders).indexOf(b.target);
                    return aIndex - bIndex;
                }));
                const firstVisibleEntry = sortedEntries.find((entry => entry.isIntersecting));
                if (firstVisibleEntry && !hintShown && !window.scrollHintManager.hasBeenShown) setTimeout((() => {
                    if (!hintShown && !window.scrollHintManager.hasBeenShown) {
                        window.scrollHintManager.show(firstVisibleEntry.target);
                        hintShown = true;
                        activeSlider = firstVisibleEntry.target;
                    }
                }), 800);
            }), {
                threshold: .3,
            ...
            tradingSliders.forEach((slider => {
                observer.observe(slider);
            }));
            globalTouchHandler = e => {
                const touchedSlider = e.target.closest(".trading-indicators__swiper");
                if (touchedSlider) hideHint();
            };
            document.addEventListener("touchstart", globalTouchHandler, {
                passive: true
            });
        }
        document.addEventListener("DOMContentLoaded", initTradingScrollHint);
        window["FLS"] = true;
        addLoadedClass();
        menuInit();
        spollers();
